<html>
<head><title>Band Favorit ~ Inputan Checkbox</title></head>
	<body>
		<FORM ACTION="prosescheckbox.php" METHOD="POST" NAME="input">
		<h2>Pilih Band Favorit Anda :</h2>
		<input type="checkbox" name="band01" value="Slank" checked> Slank<br>
		<input type="checkbox" name="band02" value="Sheila On7"> Sheila On 7<br>
		<input type="checkbox" name="band03" value="Dewa 19">Dewa 19<br>
		<input type="checkbox" name="band04" value="Pamungkas">Pamungkas<br>
		<input type="checkbox" name="band01" value="Guyon Waton" checked> Guyon Waton<br>
		<input type="submit" name="Pilih" value="Pilih">
		</FORM>
	</body>
</html>